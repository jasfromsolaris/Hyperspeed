...
### Control plane (Hyperspeed-operated; not in this repository)

Gifted **`*.hyperspeedapp.com`** DNS is implemented by a small **control-plane** service that Hyperspeed deploys on its own infrastructure. It holds `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ZONE_ID` for the zone, upserts **A** records for `{slug}.{BASE_DOMAIN}`, writes an audit log, and authenticates callers with a static bearer token. That service is **not** part of the open-source tree; customer stacks never receive Cloudflare admin credentials.

- **Endpoints (conceptual):** `POST /v1/claims` (body: `slug`, `ipv4`), `DELETE /v1/claims/{slug}` (revoke), `GET /health`.
- **Self-host default:** `docker-compose.yml` does not run the control plane. Configure **`PROVISIONING_BASE_URL`** and **`CONTROL_PLANE_BEARER_TOKEN`** on the API only when Hyperspeed provides those values for your workspace.
